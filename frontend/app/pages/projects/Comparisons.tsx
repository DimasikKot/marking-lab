...
function FileCompareColumn({
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const res = await fetchFileById(projectId, fileId, page, 10);
      if (res) setData(res);
      setLoading(false);
    };
    load();
            ...
            <div className="flex flex-wrap gap-x-1 gap-y-2">
              {row.words.map((w, wIdx) => (
                <div key={wIdx} className="flex flex-col items-center">
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded-md text-sm ${w.label !== "O" ? "bg-blue-50 text-blue-700 ring-1 ring-blue-200" : "text-gray-700"}`}
                  >
                    {w.token}
                  </span>
                </div>
