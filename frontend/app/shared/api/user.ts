...
type ValidateResponse = { status: "ok" };
...
export const validateUsername = async (
  ...
  try {
    const response = await api.post<ValidateResponse>("/users/validate-username", data);
    return response.data;
  } catch (error: unknown) {
    if (axios.isAxiosError(error)) {
      const error_text =
        error.response?.data?.detail || "Ошибка при проверке имени пользователя: " + error.message;
      toast.error(error_text);
    }
  }
};
...
export const validateEmail = async (
  ...
  try {
    const response = await api.post<ValidateResponse>("/users/validate-email", data);
    return response.data;
  } catch (error: unknown) {
    if (axios.isAxiosError(error)) {
      const error_text =
        error.response?.data?.detail || "Ошибка при проверке электронной почты: " + error.message;
      toast.error(error_text);
    }
  }
};